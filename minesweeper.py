...
def main(stdscr):
    ...
    game_win.addstr(
        0, cols * 4 + 4, f"⚑ x {flag_counter}".center(31), curses.color_pair(209)
    )
        ...
        try:
            ...
            ## key pressed is a space, essentially the whole gameplay takes there
                ...
                if _cell_ == 0:
                    ...
                    _cell_ == "*" and flag_board[y_][x_ // 4] != "F"
                ):  ## revealed cell is a mine and HASN'T been flagged
                    for i in range(cols):
                        for j in range(rows):
                            reveal(game_win, board, j, i)
                    ...
                    game_win.addstr(
                        10,
                        cols * 4 + 4,
                        f"Time elapsed: {round(time.perf_counter()-START_TIME)}s".center(
                            31
                        ),
                        curses.color_pair(178),
                    )
                    game_win.addstr(
                        rows + 3, 0, "Press any key to continue...", curses.A_BOLD
                    )
                    game_win.refresh()
                    game_win.getch()
                    DED = True
